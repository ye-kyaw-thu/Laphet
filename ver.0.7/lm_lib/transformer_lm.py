"""
Transformer Language Model for Burmese Text
Written by Ye Kyaw Thu, LU Lab., Myanmar.
Last updated: 17 Jan 2025.

This script implements a Transformer-based language model with options for training, testing, and text generation. 
It is designed for educational purposes.

How to run:

for training:
time python laphet.py --model_type transformer --train --data ./data/train/train.txt \
--model ./model/transformer.model --seq_len 50 --epochs 10 --batch_size 32 --lr 0.0001 --embedding_method nn.Embedding

for text generation:
time python laphet.py --model_type transformer --generate \
--model ./model/transformer.model --seq_len 50 --prompt "ရဲ" \
--no_of_generation 10 --embedding_method nn.Embedding

for text generation from file:
time python laphet.py --model_type transformer \
--generate --model ./model/transformer.model --seq_len 2 \
--input start_words.txt --no_of_generation 5 --output generated_texts.txt --embedding_method nn.Embedding

for testing:
time python laphet.py --model_type transformer --test --model ./model/transformer.model \
...
# Training Function
def train_transformer(model, train_dataset, dev_dataset, epochs, batch_size, lr, device, model_save_path, embedding_method, fasttext_model_path):
    """
    Train a lightweight Transformer model for next-word prediction on a tokenized dataset with validation monitoring.
    ...
    for epoch in range(epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            checkpoint = {
                ...
                "num_heads": model.layers[0].self_attn.num_heads,  # Access num_heads from MultiheadAttention
...
# Testing Function 
def test_transformer(model, tokenizer, test_file, seq_len, batch_size, device):
    """
    Evaluate the Transformer model on test data and calculate perplexity and cross-entropy.
