...
--test_file ./data/test.txt --seq_len 50 --batch_size 64 --embedding_method nn.Embedding
...
class MLP(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, seq_len, embedding_method="nn.Embedding", fasttext_model_path=None, tokenizer=None):
        ...
        if embedding_method == "nn.Embedding":
            ...
            fasttext_embeddings = torch.zeros((vocab_size, embedding_dim))
            for token, idx in tokenizer.token_to_id.items():
                if token in fasttext_vectors:
                    ...
                    fasttext_embeddings[idx] = torch.randn(embedding_dim)  # Random initialization for unknown tokens
...
# Training function
def train_mlp(model, train_dataset, dev_dataset, epochs, batch_size, lr, device, model_save_path, embedding_method, fasttext_model_path):
    """
